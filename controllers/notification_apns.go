...
func InitAPNSClient() error {
	if PushConf.Ios["Enabled"] == "true" {
		...
		if PushConf.Ios["KeyPath"] != "" {
			...
			if err != nil {
				logging.Logger.Error("Cert Error:", err.Error())
			...
			if err != nil {
				logging.Logger.Error("base64 decode error:", err.Error())
		...
		if err != nil {
			logging.Logger.Error("Transport Error:", err.Error())
...
func newApnsClient(certificate tls.Certificate) (*apns2.Client, error) {
	...
	if PushConf.Ios["Production"] == "true" {
...
func PushToIOS(req PushNotification) bool {
	logging.Logger.Debug("Start push notification for iOS")

	var (
		retryCount = 0
		maxRetry , _  = strconv.Atoi(PushConf.Ios["MaxRetry"])
	)
