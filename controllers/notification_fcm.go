...
func PushToAndroid(req PushNotification) bool {
	logging.Logger.Debug("Start push notification for Android")
	...
	if err != nil {
		logging.Logger.Error("request error: " + err.Error())
		return false
	}
...
Retry:
	...
	if err != nil {
		// FCM server error
		logging.Logger.Error("FCM server error: " + err.Error())
		return false
	}
	...
	if err != nil {
		// Send Message error
		logging.Logger.Error("FCM server send message error: " + err.Error())
		return false
	}

	if !req.IsTopic() {
		logging.Logger.Debug(fmt.Sprintf("Android Success count: %d, Failure count: %d", res.Success, res.Failure))
	}
	...
	if req.IsTopic() {
		...
		logging.Logger.Debug("Send Topic Message: ", to)
		// Success
		if res.MessageID != 0 {
			//LogPush(SucceededPush, to, req, nil)
		} else {
